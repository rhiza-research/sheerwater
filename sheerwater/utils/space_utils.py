...
def is_station_grid(ds):
    """Check if the dataset grid is a station grid, indicated by dimension station_id.
...
def check_spatial_attr(ds, grid=None, mask=None, region=None):
    ...
    if ds is None:
        return False
    if grid is not None and ds.attrs.get('grid', None) == grid:
        return True
    if mask is not None and ds.attrs.get('mask', None) == mask:
        return True
    if region is not None and ds.attrs.get('region', None) == region:
        return True
    return False
