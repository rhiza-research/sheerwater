...
from sheerwater.utils.space_utils import lon_base_change
from sheerwater.utils.secrets import postgres_write_password, postgres_read_password
    ...
    try:
        ...
        for dim in ds.coords:
            if ds[dim].dtype == object:
                ds = ds.assign_coords({dim: np.asarray(ds[dim].values, dtype=object).astype(str)})
        ds.to_zarr(store=cache_map, mode='w')
...
def cacheable(data_type, cache_args, timeseries=None, chunking=None, chunk_by_arg=None,
    ...
    def create_cacheable(func):
        ...
        def cacheable_wrapper(*args, **kwargs):
            ...
            # We must use the null path for postgres because the cache path doesn't have gs://
            if backend == 'postgres':
