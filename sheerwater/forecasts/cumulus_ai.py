...
@cache(cache=True,
       ...
       backend_kwargs={'chunking': {"lat": 300, "lon": 300, "prediction_timedelta": 10, "init_time": 25, 'member': 50}})
def cumulus_ai_daily(date, variable='precip', prob_type='deterministic',  # noqa: ARG001
    ...
    # Read directly from azure
    fs = fsspec.filesystem("az", account_name="aardvark2test", credential=credential, anon=False)
    pfcf = 'pf' if prob_type == 'probabilistic' else 'cf'
    files = [
        'az://' + x for x in fs.glob(f'az://era5-testers/ml_reforecasts/heartbreaking-portfolio/{pfcf}/total_precipitation_24h_acc/data/{dt.year}-{dt.month:02}-{dt.day:02}*')]
    if len(files) == 0:
        return None

    files = [fs.open(f) for f in files]

    ds = xr.open_mfdataset(files, engine='h5netcdf', preprocess=add_lead, parallel=True, storage_options = {"account_name": "aardvark2test", "credential": credential})
