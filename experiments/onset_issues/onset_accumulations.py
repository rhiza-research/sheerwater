...
if __name__ == "__main__":
    ...
    # get onset dates
    imerg_onset = go.get_sos_dates(datasource="imerg_final", event_name="icpac",
    ...
    # MAM column
    mam_mean.plot(ax=axs[0, 0], x="lon", vmin=0, vmax=1, cmap=cmap, add_colorbar=False)
    mam_std.plot(ax=axs[1, 0], x="lon", vmin=0, vmax=1, cmap=cmap, add_colorbar=False)
    mam_max.plot(ax=axs[2, 0], x="lon", vmin=0, vmax=1, cmap=cmap, add_colorbar=False)

    # OND column
    ond_mean.plot(ax=axs[0, 1], x="lon", vmin=0, vmax=1, cmap=cmap, add_colorbar=False)
    ond_std.plot(ax=axs[1, 1], x="lon", vmin=0, vmax=1, cmap=cmap, add_colorbar=False)
    im = ond_max.plot(ax=axs[2, 1], x="lon", vmin=0, vmax=1, cmap=cmap, add_colorbar=False)
