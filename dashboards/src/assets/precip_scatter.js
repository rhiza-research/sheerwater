...
let aggTime = String(variables.agg_time.current.value).trim().toLowerCase() === "true";
...
let timeValues = timeField
    ? Array.from(timeField.values || timeField.values.buffer || [])
    : null;

if (xValues.length !== yValues.length || xValues.length === 0) throw new Error("X and Y values must match in length and not be empty.");

// --- Aggregate over time per lat/lon if agg_time is true ---
if (aggTime) {
    let buckets = {};
    for (let i = 0; i < latValues.length; i++) {
        let key = `${latValues[i]},${lonValues[i]}`;
        if (!buckets[key]) buckets[key] = { lat: latValues[i], lon: lonValues[i], xs: [], ys: [] };
        buckets[key].xs.push(xValues[i]);
        buckets[key].ys.push(yValues[i]);
    }
    let keys = Object.keys(buckets);
    xValues   = keys.map(k => buckets[k].xs.reduce((a,v) => a+v, 0) / buckets[k].xs.length);
    yValues   = keys.map(k => buckets[k].ys.reduce((a,v) => a+v, 0) / buckets[k].ys.length);
    latValues = keys.map(k => buckets[k].lat);
    lonValues = keys.map(k => buckets[k].lon);
    timeValues = null;
}
...
if (colorby === "humidity") { colorField = series.fields.find(f => f.name === "rh2m_pct"); colorType="numeric"; }
    ...
    colorField = latValues.map((lat, i) =>
        (typeof lat === "number" && typeof lonValues[i] === "number")
            ? Math.sqrt(lat * lat + lonValues[i] * lonValues[i])
            : null
    );
    colorType = "numeric";
}
...
if (!showDensity) {
    const hoverText = latValues.map((lat, i) => {
        let base = `Lat: ${lat}<br>Lon: ${lonValues[i]}`;
        return timeValues ? `${base}<br>Time: ${new Date(timeValues[i]).toLocaleDateString()}` : base;
    });
...
if (!isNaN(event_thresh)) {
    traces.push({ x:[event_thresh,event_thresh], y:[overallMin,overallMax], type:'scatter', mode:'lines', line:{color:'orange',width:2,dash:'dot'}, showlegend:false });
    traces.push({ x:[overallMin,overallMax], y:[event_thresh,event_thresh], type:'scatter', mode:'lines', line:{color:'orange',width:2,dash:'dot'}, showlegend:false });
}
...
if (isNaN(axisLim)) axisLim = 70;

return {
    data: traces,
    layout: {
        xaxis: { title: xField.name, type: 'linear', range: [0, axisLim], autorange: false },
        yaxis: { title: yField.name, type: 'linear', range: [0, axisLim], autorange: false }
    }
};
