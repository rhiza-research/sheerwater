{
      ...
      "targets": [
        {
          ...
          "format": "table",
          "rawQuery": true,
          "rawSql": "SELECT \n  forecast,\n  AVG(CASE WHEN lead_day = 0 THEN ${metric} END) AS \"Week 1\",\n  AVG(CASE WHEN lead_day = 7 THEN ${metric} END) AS \"Week 2\",\n  AVG(CASE WHEN lead_day = 14 THEN ${metric} END) AS \"Week 3\",\n  AVG(CASE WHEN lead_day = 21 THEN ${metric} END) AS \"Week 4\",\n  AVG(CASE WHEN lead_day = 28 THEN ${metric} END) AS \"Week 5\",\n  AVG(CASE WHEN lead_day = 35 THEN ${metric} END) AS \"Week 6\"\nFROM \"$precip_tab_name\"\nWHERE COALESCE(time_grouping, 'None') IN (${time_option}) AND region = '$region_option' AND lead_day in (0, 7, 14, 21, 28, 35)\nGROUP BY forecast",
          "refId": "A",
          "sql": {
            "columns": [
              {
      ...
      "targets": [
        {
          ...
          "format": "table",
          "rawQuery": true,
          "rawSql": "SELECT \n  forecast,\n  AVG(CASE WHEN lead_day = 0 THEN ${metric} END) AS \"Week 1\",\n  AVG(CASE WHEN lead_day = 7 THEN ${metric} END) AS \"Week 2\",\n  AVG(CASE WHEN lead_day = 14 THEN ${metric} END) AS \"Week 3\",\n  AVG(CASE WHEN lead_day = 21 THEN ${metric} END) AS \"Week 4\",\n  AVG(CASE WHEN lead_day = 28 THEN ${metric} END) AS \"Week 5\",\n  AVG(CASE WHEN lead_day = 35 THEN ${metric} END) AS \"Week 6\"\nFROM \"$precip_tab_name\"\nWHERE COALESCE(time_grouping, 'None') IN (${time_option}) AND region = '$region_option2' AND lead_day in (0, 7, 14, 21, 28, 35)\nGROUP BY forecast",
          "refId": "A",
          "sql": {
            "columns": [
              {
    ...
    "list": [
      {
        ...
        "query": "MAE : mae, RMSE : rmse, ACC : acc, Bias : bias, SMAPE : smape",
        "type": "custom"
      },
      {
        "current": {
        ...
        "definition": "select \ndistinct(forecast) as __value,\ncase\n when forecast = 'salient' then 'AI-Enhanced NWP' \nwhen forecast = 'salient_gem' then 'AI-Enhanced NWP V2' \nwhen forecast = 'climatology_era5_1985_2015' then 'Clim 1985-2014' \nwhen forecast = 'climatology_imerg_1998_2024' then 'Clim IMERG' \nwhen forecast = 'climatology_imerg_1998_2016' then 'Clim IMERG' \nwhen forecast = 'climatology_trend_2015' then 'Clim + Trend' \nwhen forecast = 'climatology_rolling' then 'Clim Rolling' \nwhen forecast = 'ecmwf_ifs_er' then 'ECMWF IFS ER' \nwhen forecast = 'ecmwf_ifs_er_debiased' then 'ECMWF IFS ER Debiased' \nwhen forecast = 'ecmwf_ifs_ens' then 'ECMWF IFS ENS' \nwhen forecast = 'ecmwf_hres' then 'ECMWF HRES' \nwhen forecast = 'ecmwf_aifs' then 'ECMWF AIFS' \nwhen forecast = 'fuxi' then 'FuXi S2S' \nwhen forecast = 'gencast' then 'Gencast' \nwhen forecast = 'graphcast' then 'Graphcast' \nwhen forecast = 'gfs' then 'GFS'\n else forecast\nend as __text\nfrom \"${precip_tab_name}\" where lead_day = 0 AND ${metric} is not null",
        "includeAll": false,
        "label": "Baseline",
        "name": "baseline",
        "options": [],
      ...
      },
      {
        ...
        "query": "Countries : country, Subregions : subregion, Rainfall Region : rainfall_region, Rainfall Region+Country : country_rainfall_region",
        "type": "custom"
      },
      {
        "current": {
          "text": "Mali-West Africa Sudanian",
          "value": "mali-west_africa_sudanian"
        },
      ...
      },
      {
        ...
        "current": {
          "text": "Tanzania-East Africa Coastal Savannah",
          "value": "tanzania-east_africa_coastal_savannah"
        },
        "definition": "SELECT\n    DISTINCT initcap(replace(region, '_', ' ')) AS __text,\n    region AS __value\nFROM \"$precip_tab_name\"\nWHERE lead_day = 0",
        "label": "Second Region",
        "name": "region_option2",
      ...
      {
        "current": {
          "text": "6f98b82537bb5ab54eb13cf6fe86e328",
          "value": "6f98b82537bb5ab54eb13cf6fe86e328"
        },
        "datasource": {
          "type": "grafana-postgresql-datasource",
          "uid": "bdz3m3xs99p1cf"
      ...
      {
        "current": {
          "text": "346b7a01237d0f8eef441f27da9e1e44",
          "value": "346b7a01237d0f8eef441f27da9e1e44"
        },
        "datasource": {
          "type": "grafana-postgresql-datasource",
          "uid": "bdz3m3xs99p1cf"
  ...
  "timezone": "utc",
  "title": "Forecast Evaluation New 2",
  "uid": "cfo5l7rhhzshsc",
  "version": 1,
  "weekStart": ""
}
