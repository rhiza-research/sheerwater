      ...
      "panels": [
        {
          ...
          "id": 4,
          "options": {
            "content": "### Days Beyond Baseline\n\nFor each grid cell, take the **longest lead** L in {7, 14, 21, 28} at which that cell's `percent_good` is still at or above the Informative or Actionable cutoff (0 if it never clears the cutoff). Days beyond baseline for the cell is that horizon minus the baseline forecast's horizon on the same cell.\n\nThe table reports the **mean** of those per-cell differences over all cells in the selected region(s).\n\nBaseline row is always `0`. ★ marks the best (highest) value in each column.\n\nOther **View** options show good-cell counts by lead day instead of these summary KPIs.\n",
            "contentPartials": [],
            "defaultContent": "",
            "editor": {
              "format": "auto",
