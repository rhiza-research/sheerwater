...
if __name__ == "__main__":
    ...
    for softening in softenings:
        ...
        if not os.path.exists(f"results_{agg_days}/results_soft_{softening}"):
            os.makedirs(f"results_{agg_days}/results_soft_{softening}")
        for satellite_threshold in satellite_thresholds:
            ...
            for idx, region in enumerate(["kenya", "ghana"]):
                ...
                event_kwargs={'agg_days': agg_days},
                region=region, metric_kwargs={'soft_margin_in_days': softening}, spatial=spatial, time_grouping=None, grid=grid, recompute=True, cache_mode='overwrite')
            ...
            plt.suptitle(f"station: {station_threshold}mm | satellite: {satellite_threshold}mm | agg: {agg_days}d | softening: {softening}d")
            plt.savefig(f"results_{agg_days}/results_soft_{softening}/ber_{station_threshold}_{satellite_threshold}.png")
            plt.close()
